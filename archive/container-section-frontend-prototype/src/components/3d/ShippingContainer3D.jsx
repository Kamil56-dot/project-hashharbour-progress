...
import { OrbitControls, Environment, ContactShadows, Center, useProgress } from '@react-three/drei';
...
// ─── Phase 15 Premium Studio Lighting (Neutral Daylight 5-Point Setup) ───
function SceneLighting() {
  return (
    <>
      ...
      {/* 2. Primary Key Light — Top-Right Front (Reveals corrugation depth & structural details) */}
      <directionalLight
        position={[8, 10, 6]}
        intensity={1.65}
        color="#FFFFFF"
      ...
      {/* 3. Balanced Fill Light — Left Side (Softens harsh shadows while maintaining web contrast) */}
      <directionalLight
        position={[-7, 5, -3]}
        intensity={0.50}
        color="#E2E8F0"
      ...
      {/* 4. Subtle Rim Light — Back Top (Separates container from dark background without artificial glow) */}
      <directionalLight
        position={[-4, 7, -8]}
        intensity={0.45}
        color="#F1F5F9"
      ...
      {/* 5. Soft Top Overhead Light (Enhances roof corrugations & top edge rain gutters) */}
      <directionalLight
        position={[0, 12, 0]}
        intensity={0.35}
        color="#FFFFFF"
      ...
      {/* 6. Subtle Under Fill Light (Illuminates hollow forklift pockets & bottom chassis joists) */}
      <pointLight position={[0, -4, 0]} intensity={0.15} color="#0F172A" />
    </>
  );
}
...
// ─── Smooth Color Lerp Wrapper (250-350ms Paint Transition) ───
function SmoothColorContainer({ targetHex }) {
  const currentColorRef = useRef(new THREE.Color(targetHex));
  const targetColor = useMemo(() => new THREE.Color(targetHex), [targetHex]);
  const [animatedHex, setAnimatedHex] = useState(targetHex);

  useFrame((_, delta) => {
    currentColorRef.current.lerp(targetColor, Math.min(1, delta * 11));
    const formattedHex = '#' + currentColorRef.current.getHexString();
    if (formattedHex !== animatedHex) {
      setAnimatedHex(formattedHex);
    }
  });

  return <ContainerModel color={animatedHex} />;
}
      ...
      <div className="w-full h-full opacity-100">
        ...
        >
          <Suspense fallback={<LoadingFallback />}>
            ...
            {/* Phase 13.1 Studio Environment Polish: Soft blurred HDRI map rotated 45° for side corrugation highlights */}
            <Environment
              preset="city"
              blur={0.7}
            ...
            {/* Container model centered with smooth 300ms color transition */}
            <Center>
