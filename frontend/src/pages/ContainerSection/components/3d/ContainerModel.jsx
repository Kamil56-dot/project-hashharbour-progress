...
function createIndustrialTextures() {
  // 1. Micro-Normal & Orange Peel Bump Texture (512 x 512 Optimized)
  ...
  const imgData = ctx1.getImageData(0, 0, 512, 512);
  const data = imgData.data;
  for (let y = 0; y < 512; y++) {
    for (let x = 0; x < 512; x++) {
      const idx = (y * 512 + x) * 4;
      const f1 = Math.sin(x * 0.16 + y * 0.10) * Math.cos(y * 0.16 - x * 0.08) * 7;
      const f2 = Math.sin(x * 0.44 + 2.1) * Math.cos(y * 0.36 + 1.2) * 4;
  ...
  fadeGrad.addColorStop(0, '#858585');
  fadeGrad.addColorStop(0.3, '#7F7F7F');
  fadeGrad.addColorStop(1.0, '#7D7D7D');
  ...
  // Micro Dust Accumulation Smudges
  for (let d = 0; d < 8; d++) {
    const cx = Math.random() * 512;
    const cy = 350 + Math.random() * 140;
    const rd = 15 + Math.random() * 30;
    const dGrad = ctx2.createRadialGradient(cx, cy, 0, cx, cy, rd);
    dGrad.addColorStop(0, 'rgba(100, 95, 90, 0.06)');
...
function useMats(color) {
  ...
  const materials = useMemo(() => {
    ...
    const mats = {
      ...
      // Outer Structural Frame Rails & Corner Posts
...
function createISOMarkingsTextures() {
  ...
  ctxD.font = 'bold 32px "Courier New", "DIN 1451", monospace';
  ctxD.textAlign = 'left';

  ctxD.fillText('HHBU  749201', 70, 110);
  ctxD.strokeStyle = stencilWhite;
  ctxD.lineWidth = 3;
  ctxD.strokeRect(345, 84, 40, 40);
  ctxD.fillText('4', 356, 115);
  ctxD.font = 'bold 28px "Courier New", "DIN 1451", monospace';
  ctxD.fillText('22G1', 70, 155);
  ...
  let yOffset = 210;
  specs.forEach((item) => {
    ctxD.font = '900 21px "Courier New", "DIN 1451", monospace';
    ctxD.fillStyle = stencilMuted;
    ctxD.fillText(item.label, 70, yOffset);
    ctxD.font = '700 20px "Courier New", "DIN 1451", monospace';
    ctxD.fillText(item.metric, 240, yOffset);
    ctxD.fillText(item.imperial, 240, yOffset + 24);
    yOffset += 60;
  });

  ctxD.fillStyle = '#D97706';
  ctxD.fillRect(70, yOffset + 15, 220, 34);
  ctxD.fillStyle = '#0F172A';
  ctxD.font = '900 16px sans-serif';
  ctxD.fillText('SUPER HEAVY 20FT ISO', 82, yOffset + 38);

  const plateX = 70, plateY = 630, plateW = 360, plateH = 250;
  ...
  ctxD.lineWidth = 2;
  ctxD.strokeRect(plateX + 10, plateY + 40, plateW - 20, plateH - 50);

  ctxD.fillStyle = '#0F172A';
  ctxD.font = '900 19px sans-serif';
  ctxD.fillText('CSC SAFETY APPROVAL', plateX + 20, plateY + 30);

  ctxD.font = '700 12px monospace';
  ctxD.fillText('APPROVAL NO.: GB-LR 49201-08/2025', plateX + 20, plateY + 65);
  ctxD.fillText('DATE OF MANUFACTURE: 06 / 2025', plateX + 20, plateY + 90);
  ctxD.fillText('MANUFACTURER: HASHHARBOUR HEAVY IND.', plateX + 20, plateY + 115);
  ctxD.fillText('ALLOWABLE STACKING WT: 216,000 KG', plateX + 20, plateY + 140);
  ctxD.fillText('RACKING TEST VALUE:   150,000 N', plateX + 20, plateY + 165);
  ctxD.fillText('END-WALL STRENGTH:    0.4t', plateX + 20, plateY + 190);
  ctxD.fillText('SIDE-WALL STRENGTH:   0.6t', plateX + 20, plateY + 215);

  ctxD.fillStyle = '#334155';
  [[plateX + 9, plateY + 9], [plateX + plateW - 9, plateY + 9], [plateX + 9, plateY + plateH - 9], [plateX + plateW - 9, plateY + plateH - 9]].forEach(([cx, cy]) => {
    ctxD.beginPath();
    ctxD.arc(cx, cy, 4, 0, Math.PI * 2);
    ctxD.fill();
  });
  ...
  ctxD.fillStyle = stencilWhite;
  ctxD.font = 'bold 32px "Courier New", "DIN 1451", monospace';
  ctxD.fillText('HHBU  749201', 570, 110);
  ctxD.strokeStyle = stencilWhite;
  ctxD.lineWidth = 3;
  ctxD.strokeRect(845, 84, 40, 40);
  ctxD.fillText('4', 856, 115);
  ctxD.font = 'bold 28px "Courier New", "DIN 1451", monospace';
  ctxD.fillText('22G1', 570, 155);

  const doorLogoX = 730, doorLogoY = 410;
  drawHashHarbourSymbol(ctxD, doorLogoX, doorLogoY, 0.575, true);

  ctxD.fillStyle = '#FFFFFF';
  ctxD.font = '900 48px "Impact", "Bebas Neue", "Oswald", sans-serif';
  ctxD.textAlign = 'center';
  ctxD.fillText('#HASHHARBOUR', doorLogoX, doorLogoY + 170);

  ctxD.fillStyle = '#00C4EE';
  ctxD.font = 'bold 15px "Arial", "Montserrat", sans-serif';
  ctxD.letterSpacing = '3px';
  ctxD.fillText('SAFE  •  STRONG  •  GLOBAL', doorLogoX, doorLogoY + 200);
  ...
  // ── 2. Side Wall Markings Texture (2048 x 1024 Crisp Resolution) ──
  const sideCanvas = document.createElement('canvas');
  sideCanvas.width = 2048;
  sideCanvas.height = 1024;
  const ctxS = sideCanvas.getContext('2d');
  ctxS.clearRect(0, 0, 2048, 1024);

  const sideLogoX = 1024;
  const sideLogoY = 330;
  ...
  ctxS.font = '900 135px "Impact", "Bebas Neue", "Oswald", sans-serif';
  ctxS.letterSpacing = '10px';
  ctxS.textAlign = 'center';
  ctxS.fillText('#HASHHARBOUR', sideLogoX, 728);

  ctxS.fillStyle = '#00C4EE';
  ctxS.font = 'bold 29px "Arial", "Montserrat", sans-serif';
  ctxS.letterSpacing = '8px';
  ctxS.fillText('SAFE  •  STRONG  •  GLOBAL', sideLogoX, 775);

  ctxS.textAlign = 'right';
  ctxS.fillStyle = 'rgba(255, 255, 255, 0.90)';
  ctxS.font = 'bold 48px "Courier New", monospace';
  ctxS.fillText('HHBU  749201  4', 1940, 100);
  ctxS.fillText('22G1', 1940, 160);
...
// ─── Procedural Custom Service Stencil Texture Generator ───
function createServiceSideTexture(serviceNumber, serviceName, serviceDescription) {
  try {
    const sideCanvas = document.createElement('canvas');
    sideCanvas.width = 2048;
    sideCanvas.height = 1024;
    ...
    const sideLogoX = 1024;
    const safeNum = serviceNumber || '01';
    const safeName = (serviceName || 'CONTAINER BOOKING').toString().toUpperCase();

    // 1. ISO Code Top Right Stencil
    ctxS.textAlign = 'right';
    ctxS.fillStyle = 'rgba(255, 255, 255, 0.85)';
    ctxS.font = 'bold 44px "Courier New", monospace';
    ctxS.fillText(`ISO-${safeNum} // HASHHARBOUR`, 1940, 100);
    ctxS.fillText('22G1', 1940, 155);

    // 2. HashHarbour Symbol Icon Top Center
    if (typeof drawHashHarbourSymbol === 'function') {
      drawHashHarbourSymbol(ctxS, sideLogoX, 200, 0.9, false);
    }

    // 3. Service Number & Service Name Title (Scaled Up Bold Stencil Paint)
    ctxS.fillStyle = '#FFFFFF';
    ctxS.font = '900 115px "Impact", "Bebas Neue", "Oswald", sans-serif';
    ctxS.textAlign = 'center';
    const fullTitle = `${safeNum} — ${safeName}`;
    ctxS.fillText(fullTitle, sideLogoX, 560);

    // 4. Service Description Subtitle (Scaled Up Crisp Stencil Font below title)
    if (serviceDescription && typeof serviceDescription === 'string') {
      ctxS.fillStyle = 'rgba(255, 255, 255, 0.92)';
      ctxS.font = 'bold 40px "Arial", "Montserrat", sans-serif';

      const words = serviceDescription.split(' ');
      let line1 = '';
      let line2 = '';

      for (let i = 0; i < words.length; i++) {
        if ((line1 + words[i]).length < 42) {
          line1 += (line1 ? ' ' : '') + words[i];
        } else {
          line2 += (line2 ? ' ' : '') + words[i];
        }
      ...
      if (line1) ctxS.fillText(line1, sideLogoX, 645);
      if (line2) ctxS.fillText(line2, sideLogoX, 700);
    }
    ...
    console.error('Error creating service side texture:', err);
    return null;
  }
}
...
// ══════════════════════════════════════════
function ISOMarkings({ bumpTexture, serviceNumber, serviceName, serviceDescription }) {
  const { doorTex, sideTex } = useMemo(() => {
    const defaultMarkings = getISOMarkingsTextures();
    if (serviceNumber && serviceName) {
      const customSideTex = createServiceSideTexture(serviceNumber, serviceName, serviceDescription);
      return { doorTex: defaultMarkings.doorTex, sideTex: customSideTex };
    }
    return defaultMarkings;
  }, [serviceNumber, serviceName, serviceDescription]);
  ...
  return (
    <group>
      ...
      {/* Right Side Wall Branding (Flat unwarped overlay plane occupying 72% of height) */}
      <mesh position={[0, 0.04, sidePlaneOffset]} rotation={[0, 0, 0]} material={sideMat}>
        <planeGeometry args={[C.L * 0.92, C.H * 0.72]} />
      </mesh>

      {/* Left Side Wall Branding (Flat unwarped overlay plane occupying 72% of height) */}
      <mesh position={[0, 0.04, -sidePlaneOffset]} rotation={[0, Math.PI, 0]} material={sideMat}>
        <planeGeometry args={[C.L * 0.92, C.H * 0.72]} />
      </mesh>
    </group>
  );
